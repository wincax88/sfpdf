...
//    Add the option html5Path: "html5-min/ReaderControl.html" and html5MobilePath: "html5-min/MobileReaderControl.html" to your WebViewer
...
gulp.task('clean-heavy-files', function(callback) {
    del(['pdftronjs/pdf/PDFWorker.pexe','pdftronjs/pdf/PDFWorker.nmf','pdftronjs/pdf/pdfnet.res', 'pdftronjs/pdf/PDFNetC.js.mem'], callback);
});

gulp.task('zip-resource', () => {
    gulp.src(['lib/WebViewer.min.js']).pipe(gulp.dest('pdftronjs/'));
    var file = gulp.src(['pdftronjs/**'])
        .pipe(zip('pdftronjs.resource'))
        .pipe(gulp.dest('../src/staticresources/'));
        return file;
});


gulp.task('zip-pdfnetmem', () => {
    var file = gulp.src(['lib/html5/pdf/PDFNetC.js.mem'])
    .pipe(zip('pdfnetmem.resource'))
    .pipe(gulp.dest('../src/staticresources/'));
    return file;
        
});

gulp.task('zip-config', () => {
    var file = gulp.src(['lib/config.js'])
    .pipe(zip('pdfconfig.resource'))
    .pipe(gulp.dest('../src/staticresources/'));
    return file;
...
gulp.task('zip-pdfworker', () => {
    var file = gulp.src(['lib/html5/pdf/PDFWorker.pexe','lib/html5/pdf/PDFWorker.nmf'])
    .pipe(zip('pdfworker.resource'))
    .pipe(gulp.dest('../src/staticresources/'));
    return file;
        
});

gulp.task('zip-pdfnetres', () => {
    var file = gulp.src(['lib/html5/pdf/pdfnet.res'])
    .pipe(zip('pdfnetres.resource'))
    .pipe(gulp.dest('../src/staticresources/'));
    return file;
...
gulp.task('build', function() {
    ...
    runSequence('clean', ['copy-files', 'external-scripts', 'mobile-external-scripts', 'readercontrol-scripts', 'mobile-readercontrol-scripts', 'pdf-scripts',
         'viewer-css', 'mobile-viewer-css', 'icons-css', 'pdf-css', 'update-tags-html', 'mobile-update-tags-html'], 'move-viewer', 'zip-pdfworker', 'zip-pdfnetres', 'zip-pdfnetmem', 'clean-heavy-files', 'zip-resource');
});
